...
params:
  input_counts: "..\\data_analysis\\CM_red_filtered_counts.csv"
...
png(paste0("..\\data_analysis\\images\\WGCNA\\", params$regex, "_sample_clustering_dendrogram.png"), width = 700)
...
#png(paste0("..\\data_analysis\\images\\WGCNA\\", params$regex, "_soft_thresholding.png"))
...
#png(paste0("..\\data_analysis\\images\\WGCNA\\", params$regex, "_mean_connectivity.png"))

plot(sft$fitIndices[9:20,1], sft$fitIndices[9:20,5], xlab = "Soft Threshold (power)", ylab = "Mean connectivity", type = "n", main = paste("Mean connectivity"))
...
```{r}
write.csv(moduleTraitPvalue, paste0("..\\data_analysis\\", "WGCNA_output\\", params$regex, "_module_trait_info.csv"))
```
...
png(paste0("..\\data_analysis\\images\\WGCNA\\", params$regex, "_labeled_heatmap.png"), width = 1000, height = 700)
...
sigRows = unique(which(moduleTraitPvalue < 0.05)%%length(table(net$colors)))
...
ggsave(paste0("..\\data_analysis\\images\\WGCNA\\", params$regex, "_sig_ME_dist_boxplot.png"))
...
ggsave(paste0("..\\data_analysis\\images\\WGCNA\\", params$regex, "_sig_ME_treatment_boxplot.png"))
```
...
ggsave(paste0("..\\data_analysis\\images\\WGCNA\\", params$regex, "_sig_ME_response_scatterplot.png"), plot = arrange)
```
...
```{r}
#get modules with significant associations with categorical modules
sig_cont_names = mapply(sig_continuous, FUN = function(x){return(x[1])})
#convert index to module names from moduleTraitCor so they are in the same order as the text matrix we pulled the significance info from
sig_cont_names = rownames(moduleTraitCor)[sig_cont_names]
#remove the ME from the start and remove duplicates
sig_cont_names = unique(sapply(strsplit(sig_cont_names, "ME", fixed = TRUE), function(x) x[2]))
...
ggplot(sig_cont_tMEs, aes())
```


```{r}
ggplot(sig_tMEs, aes(x = module, y = ME, fill = reg)) + geom_boxplot() + theme(axis.text.x = element_text(angle=45, vjust=1, hjust=1, size = 10)) + ggtitle("Significant module eigengene distributions") + ylab("Module eigengene expression") + geom_hline(yintercept = 0, color = "red") + scale_fill_manual(values = c("up" = "#FF7F7F", "down" = "lightblue"))

ggsave(paste0("..\\data_analysis\\images\\WGCNA\\", params$regex, "_sig_ME_dist_boxplot.png"))

ggplot(sig_tMEs, aes(x = as.factor(treatment), y = ME)) + geom_boxplot() + facet_wrap(~module) + geom_hline(yintercept = 0, color = "red") + xlab("treatment") + ggtitle("Significant module eigengene expression by treatment")
...
MMPvalue
temp1 = subset(MMPvalue, MMPvalue$p.MMdarkgrey < 0.05, select = p.MMdarkgrey)
temp2 = subset(geneInfo, geneInfo$moduleColor == "darkgrey")
temp1$gene = rownames(temp1)
temp2$gene = rownames(temp2)
temp3 = merge(subset(MMPvalue, MMPvalue$p.MMdarkgrey < 0.05, select = p.MMdarkgrey), subset(geneInfo, geneInfo$moduleColor == "darkgrey"), all.x = TRUE)
temp1
temp2
subset(temp3, temp3$moduleColor == "darkgrey")
setdiff(unique(rownames(temp1)), unique(rownames(temp2)))
```


```{r}
write.csv(geneInfo, paste0("..\\data_analysis\\", "WGCNA_output\\", params$regex, "_module_info.csv"))


for(module in unique(sigGeneInfo$moduleColor)){
  write.csv(subset(sigGeneInfo, sigGeneInfo$moduleColor == module), paste0("..\\data_analysis\\", "WGCNA_output\\", params$regex, "_sig_module_", module, "_info.csv"))
}
...
png(paste0("..\\data_analysis\\images\\WGCNA\\", params$regex, "_sig_ME_trait_dendrogram.png"), width = 950, height = 600)
...
```{r}
#plot the heatmap matrix
png(paste0("..\\data_analysis\\images\\WGCNA\\", params$regex, "_sig_ME_trait_heatmap.png"), width = 950, height = 600)
par(cex = 1.0)
