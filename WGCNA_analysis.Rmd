...
```
```{r}
gtm = read.csv("..\\data_transfer\\pdam_gene_trans_map.csv")
gtm $gene_name = sapply(strsplit(gtm$gene_name, ".", fixed = TRUE), function (x) x[1])
sw = read.csv ("..\\data_transfer\\pdam_sw_gene_info.csv")
gtm = gtm[, -1]
names(gtm)[names(gtm) == "gene_name"] = "accession_id"
sw = sw [, -1]
annot = merge(gtm, sw)
annot$transcript_name = sapply(strsplit(annot$transcript_name, "-", fixed = TRUE), function (x) x[1])
annot 
...
probes2annot = match(probes, annot$transcript_name)
