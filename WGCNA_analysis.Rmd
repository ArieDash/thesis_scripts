...
params:
  input_counts: "..\\data_analysis\\GG_blue_filtered_counts.csv"
  input_samples: "..\\data_analysis\\samples_blue.csv"
  regex: "GG"
  genome: ""
  traits: "..\\data_analysis\\GG_traits.csv"
  sw: "..\\annotation\\gill_meta_gene_info.csv"
---
...
if(params$genome != ""){
  ...
  if(params$regex == "CL"){
    ...
    if(params$regex == "CG"){
      clust = cutreeStatic(sampleTree, cutHeight = 160000, minSize = 5)
    ...
    clust = cutreeStatic(sampleTree, cutHeight = 275000, minSize = 5)
    table(clust)
    #clust 1 contains the samples we want to keep
    keepSamples = (clust == 1)
    datExpr = dat0[keepSamples, ]
...
traitRows
#datTraits = data.frame(Ucrit_relative = datTraits)
...
net = blockwiseModules(datExpr, power = spower, minModuleSize = 30, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, verbose = 3, networkType = "signed")
```
...
labeledHeatmap(Matrix = moduleTraitCor, xLabels = names(datTraits), yLabels = names(MEs), ySymbols = names(MEs), colorLabels = FALSE, colors = colorRampPalette(c("red", "white", "blue"))(51), textMatrix = textMatrix, setStdMargins = FALSE, cex.text = 0.5, zlim = c(-1,1), main = paste("Module-trait relationships"))
...
sig_categorical = compact(mapply(sig_pos, FUN = function(x){if(x[2] <= 5){return(x)}}))
...
geneIntramodularConnectivity =  intramodularConnectivity.fromExpr(datExpr = datExpr, colors = moduleColors, networkType = "signed", getWholeNetworkConnectivity = FALSE)
...
```{r}
geneInfo = data.frame(transcript_name = probes, geneSymbol = annot$accession_id[probes2annot], moduleColor = moduleColors, geneTraitSignificance, GSPvalue, geneIntramodularConnectivity = geneIntramodularConnectivity)
...
tMEs = merge(tMEs, data.frame(sample = samples$Name, treatment = case_match(samples$Treatment, "C" ~ "control", "E" ~ "extreme", "M" ~ "moderate", "R" ~ "relaxation", "U" ~ "upwelling")))

tMEs = merge(tMEs, cbind(datTraits, sample = rownames(datTraits)))
tMEs

write.csv(tMEs, paste0("..\\data_analysis\\", "WGCNA_output\\", params$genome, params$regex, "_tMEs.csv"), row.names = FALSE)
```
...
graph_sig_cont = function(i, sig_continuous, names_list, traits_list, tMEs){
  ...
  if(nrow(sig_cont_tMEs) == 1){
    ...
    ggplot(sig_cont_tMEs, aes(x = .data[[traits_list[i]]], y = ME, color = module, shape = treatment)) + geom_point() + scale_color_manual(values = setNames(unique(sig_cont_tMEs$module), unique(sig_cont_tMEs$module)))
...
```{r}
plot_list = compact(lapply(c(6:length(colnames(moduleTraitCor))), graph_sig_cont, sig_continuous, rownames(moduleTraitCor), colnames(moduleTraitCor), tMEs))
...
```{r}
#graph of gene expression of all genes in modules of interest, with ME and specific genes highlighted

#get names of modules of interest
modules_of_interest = gsub("ME", "", unique(rownames(moduleTraitCor)[mapply(sig_continuous, FUN = function(x){x[1]})]))
#set index (need to change to for loop)
for(i in 1:length(modules_of_interest)){
  #log 2 transform and transpose expression data
  tdatExpr = as.data.frame(t(log2(datExpr)))
  tdatExpr$transcript_name = rownames(tdatExpr)
  #combine first 3 columns of gene info for module of interest and expression data
  curr_expr = merge(geneInfo[geneInfo$moduleColor == modules_of_interest[i],][,1:3], tdatExpr, by = "transcript_name")
  #convert to long format
  curr_expr = pivot_longer(curr_expr, cols = c(4:ncol(curr_expr)), names_to = "specimen", values_to = "expression")
  
  #get gene info of current module of interest
  genes_curr_of_interest = select(geneInfo, transcript_name, paste0("MM.", modules_of_interest[i]), moduleColor)
  #select out only those genes actually in the module
  genes_curr_of_interest = subset(genes_curr_of_interest, genes_curr_of_interest$moduleColor == modules_of_interest[i])
  #set name of MM.[module color] column to just MM
  colnames(genes_curr_of_interest)[colnames(genes_curr_of_interest) == paste0("MM.", modules_of_interest[i])]  = "MM"
  #sort by MM for that module
  genes_curr_of_interest =  genes_curr_of_interest[order(genes_curr_of_interest$MM, decreasing = TRUE),]
  #select only the top 5 names
  genes_curr_of_interest = genes_curr_of_interest[1:5,]$transcript_name
  
  ggplot(curr_expr, aes(x = specimen, y = expression, group = transcript_name)) + geom_line(alpha = 0.2) + geom_line(data = subset(curr_expr, curr_expr$transcript_name %in% genes_curr_of_interest), aes(color = transcript_name), linewidth = 0.5) + scale_x_discrete(guide = guide_axis(n.dodge = 2))
  
  ggsave(paste0(paste0("..\\data_analysis\\images\\WGCNA\\", params$genome, params$regex, "_", modules_of_interest[i],  "_total_expression_graph.png")))
