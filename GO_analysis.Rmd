...
params:
  gtm: "..\\annotation\\gill_meta_accession_trans_map_new.csv"
  de: "..\\data_analysis\\DE_lists\\CG_blue_UvsC_P0.05C1_DE.csv"
  counts: "..\\data_analysis\\CG_blue_txi.csv"
  info: "..\\annotation\\gill_meta_gene_info.csv"
  baseout : "..\\data_analysis\\images\\CG_blue\\CG_blue_UvsC_DE_"
  listout : "..\\data_analysis\\GO_lists\\CG_blue_UvsC_P0.05C1_DE_"
  kegg : 0
---
...
assayed_genes = gtm$accession_id
de_genes = degs$accession_id
...
ave_len = ave_len[names(ave_len) %in% gtm$accession_id]
```
...
#go_terms$over_represented_qvalue = p.adjust(go_terms$over_represented_pvalue, method = "fdr")
#go_terms$under_represented_qvalue = p.adjust(go_terms$under_represented_pvalue, method = "fdr")
#write.csv(go_terms, paste0(params$listout, "GO_list.csv"))
...
```{r}
test_by_ont = function(pwf, gene2cat, test.cats, method = "Wallenius"){
  
  ontology = unlist(strsplit(test.cats, ":"))[2]
  
  res = goseq(pwf = pwf, gene2cat = gene2cat, test.cats = test.cats, method = method)
  res$over_represented_qvalue = p.adjust(res$over_represented_pvalue, method = "fdr")
  res$under_represented_qvalue = p.adjust(res$under_represented_pvalue, method = "fdr")
  write.csv(res, paste0(params$listout, ontology, "_total_GO_list.csv"))
  
  res = subset(res, res$over_represented_qvalue <= 0.05 | res$under_represented_qvalue <= 0.05)
  
  write.csv(res, paste0(params$listout, ontology, "_sig_GO_list.csv"))
}
```

```{r}
onts = c("GO:CC", "GO:BP", "GO:MF")
for(ont in onts){
  test_by_ont(pwf, gene2cat, ont)
}
```
...
if(params$kegg){
  ...
  gene2cat = gene_info[,c("gene_name", "path_desc")]
