...
```{r}
#if ran DIAMOND with k > 1, use this chunk to eliminate extraneous info from gene name column
sp
sp$gene_name = unname(sapply(sp$gene_name, FUN = function(x){unlist(strsplit(x, "|", fixed = TRUE))[2]}))
sp
```
...
```{r}
sp_filter = sp[sp$gene_name %in% ac,]
...
write.csv(sp_filter, "..\\annotation\\Zuri_oases_gene_trans_map_new.csv")
```
