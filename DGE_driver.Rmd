...
for(regex in tissues){
  for(type in types){
    if(genome == TRUE){
      ...
      out = paste0("..\\data_analysis\\genome", regex, "_", type, "_txi.csv")
    ...
    rmarkdown::render(input = "tximport.Rmd", params = list(out = out, samples_location = samples_location, input_samples = input_samples, regex = regex))
...
for(type in types){
  for(tissue in tissues){
    sig_modules = list.files("..\\data_analysis\\WGCNA_output\\", pattern = paste0(tissue, "_sig"), full.names = TRUE)
    for(sig_module in sig_modules){

       counts = paste0("..\\data_analysis\\", tissue, "_", type, "_txi.csv")
       if(strsplit(sig_module, "_")[[1]][7] == "top"){
         baseout = paste0("..\\data_analysis\\images\\WGCNA\\", tissue, "_", type, "\\", tissue, "_", type, "_", strsplit(sig_module, "_")[[1]][6], "_top_")
         
         listout = paste0("..\\data_analysis\\GO_lists\\WGCNA\\", tissue, "_", type, "_", strsplit(sig_module, "_")[[1]][4], "_top_")
       }else{
          baseout = paste0("..\\data_analysis\\images\\WGCNA\\", tissue, "_", type, "\\", tissue, "_", type, "_", strsplit(sig_module, "_")[[1]][6], "_")
          
          listout = paste0("..\\data_analysis\\GO_lists\\WGCNA\\", tissue, "_", type, "_", strsplit(sig_module, "_")[[1]][4], "_")
       }
       
       
       
       xfun::Rscript_call(rmarkdown::render, list(input = "GO_analysis.Rmd", params = list(de = sig_module, counts = counts, baseout = baseout, listout = listout)))
    }
  }
}
```
...
info = "..\\annotation\\sw_kegg_info.csv"
...
for(type in types){
  for(tissue in tissues){
    ...
    for(sig_module in sig_modules){
       ...
       xfun::Rscript_call(rmarkdown::render, list(input = "GO_analysis.Rmd", params = list(de = sig_module, counts = counts, info = info, baseout = baseout, listout = listout, kegg = 1)))
    }
  }
}
```
