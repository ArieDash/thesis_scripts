...
for(regex in tissues){
  for(type in types){
    ...
    out = paste0("..\\data_analysis\\DE_lists\\", regex, "_", type, "_")
    ...
    xfun::Rscript_call(rmarkdown::render, list(input = "DGE_glm.Rmd", params = list(input_samples = input_samples, regex = regex, input_counts = input_counts, install = install, out = out), output_file = report_output, output_format = 'pdf_document'))
