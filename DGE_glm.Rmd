...
if("E" %in% samples$Treatment){
  contrasts = makeContrasts(UvsC=U-C, RvsC=R-C, UvsR=U-R, UvsE=U-E, UvsM=U-M, RvsM=R-M, MvsC=M-C, EvsC=E-C, EvsM=E-M, EvsR=E-R, levels = design)
} else{
...
calc_deg = function(fit, test, anno_color_opts, color){
  ...
  if(length(rownames(sig_genes)) > 1){
    ...
    make_heatmap(cpms = cpms, samples_comp = samples, sig_genes = rownames(sig_genes), main = paste0(test, " significant DEG heatmap"), color = color, annotation_color = list(treatment = anno_color_opts), filename = paste0(params$image_out, test, "_deg_heatmap_all_samples.png"))
...
for(test in tests){
  ...
  sig_genes = unique(c(sig_genes, rownames(res)))
...
make_heatmap(cpms = cpms, samples_comp = samples, sig_genes = sig_genes, main = "Significant DEG heatmap", color = cols, annotation_color = list(treatment = anno_color_opts), filename = paste0(params$image_out, "sig_deg_heatmap_all_samples.png"))
...
if(length(sig_genes_shared) > 1){
  
  anno_color = list(treatment = anno_color_opts)

  make_heatmap(cpms = cpms, samples_comp = samples, sig_genes = sig_genes_shared, main = "shared DEG heatmap", color = cols, annotation_color = anno_color, filename = paste0(params$image_out, "shared_deg_heatmap.png"))
...
#  write_sig_genes(sig_genes_UE, "UvsE")
  ...
  ##FvsS
  contrasts = makeContrasts(FvsS=(U+R)/2-(C+E+M)/3, levels = design)
  ...
  #make heatmap if more than 1 DEG
  if(nrow(sig_genes_FS) > 1){
    make_heatmap(cpms = cpms, samples_comp = samples, sig_genes = rownames(sig_genes_FS), main = "FvsS DEG heatmap", color = cols, annotation_color = anno_color, filename = paste0(params$image_out, "FvsS_deg_heatmap.png"))
  }
